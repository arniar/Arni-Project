...
const prevRecommendation = document.querySelector('.personal-recommendations .prev');
const nextRecommendation = document.querySelector('.personal-recommendations .next');

let scrollPosition = 0;
const cardWidth = 300; // card width + gap

prevRecommendation.addEventListener('click', () => {
    scrollPosition = Math.max(scrollPosition - cardWidth, 0);
    recommendationsSlider.style.transform = `translateX(-${scrollPosition}px)`;
});

nextRecommendation.addEventListener('click', () => {
    const maxScroll = (recommendationCards.length - 3) * cardWidth;
